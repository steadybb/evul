...
# Updated with enhanced logger integration and proxy bypass for Microsoft endpoints
...
class StealthEngine:
    ...
    - Proxy bypass for Microsoft endpoints
    """
    
    # Microsoft endpoints that should bypass proxies to avoid 403 errors
    MICROSOFT_ENDPOINTS = [
    ...
    def __init__(self, proxy_list: Optional[List[str]] = None, min_jitter: float = 1.0, max_jitter: float = 3.5, use_manager_proxy: bool = True):
        ...
        Args:
            ...
            use_manager_proxy: Whether to use the centralized proxy manager
        """
    ...
    def _should_bypass_proxy(self, url: str) -> bool:
        """Check if the URL should bypass the proxy (e.g., Microsoft endpoints)."""
        if not url:
            return False
        
        url_lower = url.lower()
        for endpoint in self.MICROSOFT_ENDPOINTS:
            if endpoint in url_lower:
                logger.debug(f"Bypassing proxy for Microsoft endpoint: {endpoint}")
                return True
        return False
    ...
    def build_session(self) -> requests.Session:
        ...
        Automatically bypasses proxies for Microsoft endpoints to avoid 403 errors.
        """
        ...
        if proxy:
            ...
            # Monkey-patch the request method to handle proxy bypass
            ...
            def patched_request(method, url, *args, **kwargs):
                ...
                if self._should_bypass_proxy(url):
                    # Temporarily remove proxy for this request
                    original_proxies = sess.proxies.copy()
                    sess.proxies = {}
                    try:
                        logger.debug(f"Bypassed proxy for Microsoft endpoint: {url}")
                        return original_request(method, url, *args, **kwargs)
                    finally:
                        # Restore proxies
                        sess.proxies = original_proxies
                else:
                    return original_request(method, url, *args, **kwargs)
            
            sess.request = patched_request
            logger.proxy(f"Session {self._session_count} using proxy: {proxy_url} (Microsoft endpoints will bypass proxy)")
        else:
            self._last_selected_proxy = None
